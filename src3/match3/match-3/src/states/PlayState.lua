...
function PlayState:update(dt)
    ...
    if self.canInput then
        ...
        if love.mouse.wasPressed(1) then
            ...
            for y1 = 1, 8 do
                for x1 = 1, 8 do
                    local tile = self.board.tiles[y1][x1]
                    if x > tile:getX() + self.board.x and x < tile:getX() + self.board.x + 32 then
                        if y > tile:getY() + self.board.y and y < tile:getY() + self.board.y + 32 then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') or mouseclick then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                :finish(function()
                    if not self.board:calculateMatches() then
                            -- swap back because it is an invalid move
                                
                        --self.issue = true
                                -- swap grid positions of tiles
                        local tempX1 = newTile.gridX
                        local tempY1 = newTile.gridY

                        newTile.gridX = self.highlightedTile.gridX
                        newTile.gridY = self.highlightedTile.gridY
                        self.highlightedTile.gridX = tempX1
                        self.highlightedTile.gridY = tempY1
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            local extraScore = 0
            for l, match2 in pairs(match) do
                extraScore = extraScore + match2:getVariety() * 10
            end

            self.score = self.score + #match * 50 + extraScore
    ...
    while not self.board:getPossibility() do
...
function PlayState:render()
    ...
    if self.issue then
        love.graphics.printf('Issue! ', 20, 124, 182, 'center')
    end
end

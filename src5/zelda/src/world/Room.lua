...
function Room:update(dt)
    ...
    for i = #self.entities, 1, -1 do
        ...
        if entity.health <= 0 then
            if math.random(2) == 1 and not entity.dead then
    ...
    for y = 1, self.height do
        for x = 1, self.width do
            ...
            love.graphics.draw(gTextures['tiles'], gFrames['tiles'][tile.id],
                math.ceil((x - 1) * TILE_SIZE + self.renderOffsetX + self.adjacentOffsetX), 
                math.ceil((y - 1) * TILE_SIZE + self.renderOffsetY + self.adjacentOffsetY))
        end
    end

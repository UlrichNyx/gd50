...
function AlienLaunchMarker:update(dt)
    ...
    if self.alien and not self.alien.touched and love.keyboard.wasPressed('space') and not self.alien.split then
        self.alien.split = true
        self.clone1 = Alien(self.world, 'round', self.alien.body:getX(), self.alien.body:getY() + 30, 'Player')
        self.clone1.body:setLinearVelocity(self.alien.body:getLinearVelocity())
        self.clone1.fixture:setRestitution(0.4)
        self.clone1.body:setAngularDamping(5)
        self.clone2 = Alien(self.world, 'round', self.alien.body:getX(), self.alien.body:getY() - 30, 'Player')
        self.clone2.body:setLinearVelocity(self.alien.body:getLinearVelocity())
        self.clone2.fixture:setRestitution(0.4)
        self.clone2.body:setAngularDamping(5)
    end
    ...
    if not self.launched then
        ...
        elseif love.mouse.wasReleased(1) and self.aiming then
            ...
            self.alien.body:setAngularDamping(5)
...
function AlienLaunchMarker:render()
    if not self.launched then
        ...
        if self.alien and self.alien.split then
            self.clone1:render()
            self.clone2:render()
        end
    end
